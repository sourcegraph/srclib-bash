...
ifeq ($(OS),Windows_NT)
	SRCLIB_BASH_EXE := .bin/srclib-bash.exe
	...
	SRCLIB_BASH_EXE := .bin/srclib-bash
...
install: ${SRCLIB_BASH_EXE}

govendor:
	go get github.com/kardianos/govendor
	govendor sync

${SRCLIB_BASH_EXE}: $(shell /usr/bin/find . -type f -and -name '*.go' -not -path './vendor/*')
	go build -o ${SRCLIB_BASH_EXE}
